...
const Community = () => {
    ...
    // friends (mock — swap with real socket/friends API if available)
    const mockFriends = [
        { name: 'Yellow', online: true },
        { name: 'Lisa', online: true },
        { name: 'PP', online: false },
    ...
    const handleLike = async (postId) => {
        ...
        try {
            ...
            setPosts(prev => prev.map(p => {
                ...
                return { ...p, likes: liked ? p.likes.slice(0, -1) : [...p.likes, currentUser._id] };
            }));
        } catch (err) {
        ...
        <div className="min-h-screen bg-[#05050f] text-white font-sans">
                ...
                <div className="max-w-7xl mx-auto flex items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-5 w-auto justify-end">
                        <Link to="/shops" className="hidden md:flex items-center gap-2 text-gray-300 hover:text-[#8b2cf5] font-medium transition-colors mr-2">
                            <Store className="w-5 h-5" /> ร้านค้า
                        </Link>
                                    ...
                                    {showDropdown && (
                                        <div className="absolute right-0 top-12 w-48 bg-[#12121e] border border-[#2a2a3e] rounded-xl shadow-2xl overflow-hidden z-50">
                                            ...
                                            <div className="p-2">
                                                ...
                                                <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-400 hover:bg-red-500/10 hover:text-red-300 rounded-lg transition-colors mt-1">
            ...
            <div className="bg-gradient-to-r from-[#1c0d33] via-[#0e0a20] to-[#05050f] border-b border-[#2a2a3e] py-5 px-4">
                <div className="max-w-7xl mx-auto flex items-center gap-3">
                    <div className="p-2.5 bg-[#8b2cf5]/20 rounded-xl border border-[#8b2cf5]/30">
                        <Users className="w-6 h-6 text-[#8b2cf5]" />
                    </div>
                    <div>
                        <h1 className="text-xl font-bold text-white">ชุมชนนักแลก</h1>
                        <p className="text-xs text-gray-400">พูดคุย แชร์ และหาของแลกกัน</p>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 py-6 flex gap-5">
                ...
                <div className="flex-1 min-w-0 relative">
                    ...
                    <div className="flex items-center gap-2 mb-5 overflow-x-auto pb-1 scrollbar-none">
                        ...
                        {Object.entries(POST_TYPE_LABELS).map(([key, { label, icon }]) => (
                            <button
                                ...
                                className={`flex items-center gap-1.5 px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-all border ${activeFilter === key
                                    ? 'bg-[#8b2cf5] border-[#8b2cf5] text-white shadow-[0_0_12px_rgba(139,44,245,0.5)]'
                    ...
                    {/* Posts + FAB wrapped in one relative box */}
                    <div className="relative bg-[#12121e] rounded-xl border border-[#2a2a3e] min-h-[300px] pb-16">
                        {loading ? (
                            <div className="flex justify-center items-center py-24">
                                <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-[#8b2cf5]" />
                            </div>
                        ) : posts.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-24 text-center">
                                <MessageCircle className="w-14 h-14 mb-4 text-[#2a2a3e]" />
                                <p className="text-gray-300 font-medium">ยังไม่มีโพสต์ในตอนนี้</p>
                                <p className="text-sm text-gray-500 mt-1">เป็นคนแรกที่โพสต์ในชุมชนนี้เลย!</p>
                            </div>
                        ) : (
                        ...
                        {/* FAB — absolute bottom-right inside the box */}
                        <div className="absolute bottom-4 right-4">
                            <button
                                onClick={() => currentUser ? setShowCreateModal(true) : navigate('/login')}
                                title="เพิ่มโพสต์ใหม่"
                                className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-[#8b2cf5] to-[#4361ee] text-white font-semibold text-sm shadow-[0_0_20px_rgba(139,44,245,0.5)] hover:shadow-[0_0_28px_rgba(139,44,245,0.8)] hover:scale-105 active:scale-95 transition-all"
                            >
                ...
                <aside className="w-64 shrink-0 hidden lg:block">
                    <div className="bg-[#12121e] rounded-xl border border-[#2a2a3e] p-4 sticky top-24">
                        <div className="flex items-center gap-2 mb-4">
                            <Users className="w-4 h-4 text-[#8b2cf5]" />
                            <span className="font-semibold text-sm text-gray-200">เพื่อนออนไลน์</span>
                        </div>
                        <div className="flex flex-col gap-3">
                            {mockFriends.map(f => (
                                <div key={f.name} className="flex items-center gap-3 cursor-pointer group hover:bg-[#1c1c2b] rounded-lg p-2 transition-colors -mx-2">
                                    <div className="relative">
                                        <Avatar name={f.name} size={9} />
                                        <span className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-[#12121e] ${f.online ? 'bg-orange-400' : 'bg-gray-600'}`} />
                                    </div>
                                    <div>
                                        <p className="text-sm font-medium text-gray-200 group-hover:text-white transition-colors">{f.name}</p>
                                        <p className="text-xs text-gray-500">{f.online ? 'ออนไลน์' : 'ออฟไลน์'}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                        <div className="mt-4 pt-4 border-t border-[#2a2a3e]">
                            <Link to="/chat" className="flex items-center justify-center gap-2 w-full py-2 rounded-lg bg-[#1c1c2b] hover:bg-[#8b2cf5]/20 hover:text-[#8b2cf5] text-gray-400 text-sm transition-colors border border-[#2a2a3e] hover:border-[#8b2cf5]/40">
            ...
            {showCreateModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
                    <div className="w-full max-w-lg bg-[#12121e] border border-[#2a2a3e] rounded-2xl shadow-2xl overflow-hidden">
                        {/* Modal Header */}
                        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2a2a3e] bg-[#0a0a16]">
                            <div className="flex items-center gap-2">
                                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#8b2cf5] to-[#4361ee] flex items-center justify-center">
                                    <Plus className="w-4 h-4 text-white" />
                                </div>
                                <h2 className="font-bold text-white">สร้างโพสต์ใหม่</h2>
                            </div>
                            <button onClick={() => setShowCreateModal(false)} className="p-1 hover:bg-[#2a2a3e] rounded-lg transition-colors">
                        ...
                        <form onSubmit={handleCreatePost} className="p-6 flex flex-col gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                <Avatar name={currentUser?.username} size={10} />
                                <div>
                                    <p className="font-semibold text-white text-sm">{currentUser?.username}</p>
                                    <p className="text-xs text-gray-500">กำลังโพสต์ในชุมชน</p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {Object.entries(POST_TYPE_LABELS).filter(([k]) => k !== 'ALL').map(([key, { label, icon }]) => (
                                        <button
                                            type="button" key={key}
                                            onClick={() => setForm(f => ({ ...f, postType: key }))}
                                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${form.postType === key
                                                ? 'bg-[#8b2cf5] border-[#8b2cf5] text-white'
                                                : 'bg-[#1c1c2b] border-[#2a2a3e] text-gray-400 hover:border-[#8b2cf5]/50'
                                                }`}
                                        >
                                            {icon}{label}
                                        </button>
                            ...
                            <div>
                                <textarea
                                    ...
                                    placeholder="เขียนอะไรก็ได้ที่อยากแชร์กับชุมชน..."
                                    className="w-full bg-[#0a0a16] border border-[#2a2a3e] rounded-xl p-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#8b2cf5] resize-none transition-colors"
                                />
                            </div>

                            {/* Tags */}
                            <div>
                                <label className="text-xs text-gray-400 mb-1.5 block">แท็ก (คั่นด้วยจุลภาค)</label>
                                <input
                                    type="text"
                                    value={form.tags}
                                    onChange={e => setForm(f => ({ ...f, tags: e.target.value }))}
                                    placeholder="#หาของ, #รีวิว"
                                    className="w-full bg-[#0a0a16] border border-[#2a2a3e] rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#8b2cf5] transition-colors"
                                />
                            </div>

                            {formError && (
                                <p className="text-sm text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-4 py-2">{formError}</p>
                            )}

                            <div className="flex justify-end gap-3 pt-2">
                                <button type="button" onClick={() => setShowCreateModal(false)}
                                    className="px-5 py-2 rounded-lg bg-[#1c1c2b] text-gray-400 hover:text-white border border-[#2a2a3e] text-sm transition-colors">
                                    ยกเลิก
                                </button>
                                <button type="submit" disabled={submitting}
                                    className="flex items-center gap-2 px-6 py-2 rounded-lg bg-gradient-to-r from-[#8b2cf5] to-[#4361ee] text-white font-medium text-sm hover:opacity-90 transition-opacity disabled:opacity-50">
...
function PostCard({ post, currentUser, liked, onLike }) {
    ...
    return (
        <div className="bg-[#12121e] rounded-xl border border-[#2a2a3e] hover:border-[#8b2cf5]/40 transition-all duration-300 overflow-hidden group">
            {/* Card Header */}
            <div className="flex items-start gap-3 p-4">
                <Avatar name={post.author?.username} src={post.author?.imageProfile} size={10} />
                <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                        <span className="font-semibold text-white text-sm">{post.author?.username || 'Unknown'}</span>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${badgeClass}`}>{label}</span>
                        <span className="text-xs text-gray-500">{timeAgo(post.createdAt)}</span>
                        {post.status === 'EDITED' && <span className="text-[10px] text-gray-600 italic">(แก้ไขแล้ว)</span>}
                    </div>
                    {/* Tags */}
                    {post.tags?.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                            {post.tags.map((t, i) => (
                                <span key={i} className="text-[10px] text-[#8b2cf5] bg-[#8b2cf5]/10 rounded px-1.5 py-0.5">#{t}</span>
                            ))}
                        </div>
                    )}
                </div>
            ...
            <div className="px-4 pb-3">
                <p className={`text-gray-200 text-sm leading-relaxed ${!expanded && 'line-clamp-4'}`}>
                    {post.content}
                </p>
                {post.content?.length > 250 && (
                    <button onClick={() => setExpanded(!expanded)} className="text-[#8b2cf5] text-xs mt-1 hover:underline">
            ...
            {post.images?.length > 0 && (
                <div className={`px-4 pb-3 grid gap-2 ${post.images.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
                    {post.images.slice(0, 4).map((img, i) => (
                        <div key={i} className="relative rounded-lg overflow-hidden bg-[#1c1c2b] aspect-video">
            ...
            {/* Referenced Product */}
            {post.referencedProduct && (
                <div className="mx-4 mb-3 flex items-center gap-3 bg-[#1c0d33] border border-[#8b2cf5]/20 rounded-xl p-3">
                    <div className="w-12 h-12 rounded-lg bg-[#2a1a4e] overflow-hidden shrink-0 flex items-center justify-center">
                        {post.referencedProduct.images?.[0]
                            ? <img src={post.referencedProduct.images[0]} alt="" className="w-full h-full object-cover" />
                            : <Image className="w-5 h-5 text-[#8b2cf5]" />}
                    </div>
                    <div>
                        <p className="text-xs text-[#8b2cf5] font-semibold mb-0.5">สินค้าที่แนบ</p>
                        <p className="text-sm text-white font-medium line-clamp-1">{post.referencedProduct.productName}</p>
                        {post.referencedProduct.price && (
                            <p className="text-xs text-gray-400">฿{post.referencedProduct.price.toLocaleString()}</p>
                        )}
                    </div>
                </div>
            )}
            ...
            <div className="flex items-center gap-4 px-4 py-3 border-t border-[#1c1c2b]">
                <button
                    onClick={onLike}
                    className={`flex items-center gap-1.5 text-sm transition-colors ${liked ? 'text-[#8b2cf5]' : 'text-gray-500 hover:text-[#8b2cf5]'}`}
                >
